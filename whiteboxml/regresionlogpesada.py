...
Original file is located at
    https://colab.research.google.com/drive/1YD2BxHb5CT9Zg6wPUhtRP2nJcJdn_mrr
"""
...
class RegresionLogPesada :
    ...
    def _Fit(self, X, y, x_objetivo, max_iter=80, tol=1e-6, lr=0.05) -> np.ndarray:
        """
    ...
    def _predict_proba(self, X, y, x_objetivo) -> float:
        """
        ...
        params = self._Fit(X, y, x_objetivo)
        return 1 / (1 + np.exp(-(x_objetivo @ params)))

    def _predict_clase(self, X, y, x_objetivo, umbral=0.5) -> int:
        """
        ...
        :date: 13/06/2026
        """
        return int(self._predict_proba(X, y, x_objetivo) > umbral)
...
Z = np.array([modelo._predict_clase(X, y, punto) for punto in grid])
...
probas = np.array([modelo._predict_proba(X, y, p) for p in grid]).reshape(xx1.shape)
